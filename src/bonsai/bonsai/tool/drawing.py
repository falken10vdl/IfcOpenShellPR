...
class Drawing(bonsai.core.tool.Drawing):
    ...
    @classmethod
    def create_svg_document(cls, document: ifcopenshell.entity_instance) -> None:
        """Regenerate the SVG backing a sheet document, if it can be authored.

        Only schedules are generated by Bonsai; a REFERENCE document is an
        externally supplied SVG that callers already require to exist before
        reaching this point, so there is nothing to (re)create for it.
        """
        if document.is_a("IfcDocumentInformation"):
            cls.create_svg_schedule(document)
